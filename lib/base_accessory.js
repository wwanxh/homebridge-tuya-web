...
class BaseAccessory {
    ...
    updateAccessory(device) {
        ...
        if (device.name) {
            ...
            var setCharacteristic = (characteristic, value) => {
                var char = accessoryInformationService.getCharacteristic(characteristic) ||
            ...
            setCharacteristic(Characteristic.Model, Accessory.Categories[this.categoryType]);
        }
    ...
    setCharacteristic(characteristic, value, updateHomekit) {
        updateHomekit && this.service
    ...
    onIdentify() {
        this.log('[IDENTIFY] %s', this.homebridgeAccessory.displayName)
    }
}
